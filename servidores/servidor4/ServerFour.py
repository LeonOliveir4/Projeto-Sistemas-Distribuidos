    ...
    try:
        ...
        try:
            ...
            if "backup" in response_data:
                backup_server_info = tuple(response_data["backup"])
                print(f"Informações do servidor de backup armazenadas: {backup_server_info}")
                connectionSocket.close()
                return
        except json.JSONDecodeError:
            pass  # Continua o processamento se não for JSON
        ...
        # Enviar o arquivo para o servidor de backup, se as informações estiverem disponíveis
        if backup_server_info:
            send_backup(fileName, data, backup_server_info)
        else:
...
def getServerStatus():
    ...
    return f"CPU: {cpuUsage}%, Memory: {memUsage}%" 
...
def get_backup_server(middlewareHost, middlewarePort):
    try:
        clientSocket = socket(AF_INET, SOCK_STREAM)
        clientSocket.connect((middlewareHost, middlewarePort))
        clientSocket.sendall(b"GET_BACKUP_SERVER")
        response = clientSocket.recv(1024)
        backup_info = json.loads(response.decode())
        clientSocket.close()
        return tuple(backup_info["backup"])
    except Exception as e:
        print(f"Erro ao obter servidor de backup: {e}")
        return None

def send_backup(fileName, data, backup):
    try:
        clientSocket = socket(AF_INET, SOCK_STREAM)
        ...
        clientSocket.close()
        print(f"Backup do arquivo {fileName} enviado ao servidor de backup {backup[0]}:{backup[1]}")
    except Exception as e:
        print(f"Erro ao enviar backup do arquivo {fileName} ao servidor {backup[0]}:{backup[1]}: {e}")
...
def main():
    ...
    configPath = os.path.join(currentDir, 'configServerFour.json')
